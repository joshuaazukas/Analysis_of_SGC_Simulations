...
mse = sum((data_array_filtered .- gen_beta_dis).^2) / length(data_array);

# Calculate Root Mean Squared Error (RMSE)
rmse = sqrt(mse)

# Perform the Kolmogorov-Smirnov (KS) test
ks_test = ApproximateTwoSampleKSTest(gen_beta_dis, data_array_filtered)

# Extract the p-value from the test result
p_value_ks = pvalue(ks_test)
# Display the p-value
