...
p = (kOn => 0.0033*60*60, kOff => 0.0033*60*60,  kTr => 310, kTl => 425, dM => 1, dG => 0.00365) #parameter set for NO FEEDBACK Circuit
ukOn = Normal(0.0033*60*60,0.5*0.0033*60*60) # standard deviation raised to 20%. estimated from Suter et al. paper that used/measured kon and koff values (no reported standard deviation)
ukOff = Normal(0.0033*60*60,0.5*0.0033*60*60)
ukTr = Normal(310,150)
udG = Normal(0.00365,0.00126);
# calculate gamma distribution for kTL with mean=310 and std=265
g_scale = 100^2/425  #adjusted from 265^2/500
g_shape = 425/g_scale
...
plot(kTr_x, kTr_y, label="kTr", ylabel="PDF", xlabel="kTr (Hr^-1)")
...
for i in 1:1000
    ...
    kTr_new = bounded_sample_min0(ukTr)
    push!(dist_kTr,kTr_new)  
    kTr_newg = bounded_sample_min0(ukTrg)
    push!(dist_kTrg,kTr_newg)
    dG_newg = bounded_sample_min0(udG_ln)
    push!(dist_dG,dG_newg)
end
...
histogram(dist_dG,bins=200,label="dG:$(round(mean(dist_dG), digits=4)),$(round(std(dist_dG), digits=4)) ")
...
histogram(dist_b_kTl,bins=200, label="kTkl",ylabel="counts",xlabel="kTl")
histogram(dist_b_kTr,bins=200,label="kTr",ylabel="counts",xlabel="kTr")
...
plot(sol, idxs=3,label="DNAon",xlims=(1500,1572))
plot(sol, idxs=2,label="DNAoff",xlims=(1500,1572))
slope = ((GFP1[end]-GFP1[1])/(time[end]-time[1]))

dir_path = "C:/Users/jsazu/Documents/GitHub/Analysis_of_SGC_Simulations/StreyCats/Broken Feedback Circuit/Simple_degG_3/"
...
savefig(joinpath(dir_path, "kTr.png"))
...
jldsave("C:/Users/jsazu/Documents/GitHub/Analysis_of_SGC_Simulations/StreyCats/Broken Feedback Circuit/Simple_degG_2/Simple_degG_3.jld2"; sim_sumstats,simn_sumstats,params)
