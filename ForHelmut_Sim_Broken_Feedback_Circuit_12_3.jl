...
for i in 1:1000
    ...
    kTl_new = bounded_sample(ukTl, max_value, 100)
    push!(dist_b_kTl,kTl_new)
end
dist_b_kTr=[]
for i in 1:1000
    # Generate a new sample
    kTr_new = bounded_sample(ukTr, max_value, 50)
...
plot(GFP1)
plot(sol, idxs=3,label="Activator",xlim=(0,20),xlabel="t in hours")
savefig("activator.png")
...
@time for i in 1:10
    ...
    kOn_new = rand(ukOn)
    kOff_new = rand(ukOff)
    kTr_new = bounded_sample(ukTr, 600, 50)
    kTl_new = bounded_sample(ukTl, max_value, 100)
...
jldsave("run1bfc.jld2"; sim_sumstats,params)
