...
for i in 1:cols
    ...
    exp_mGFP=mean(ex_GFP[:,i])
    push!(exp_temp_mean, exp_mGFP)
end
scatter(exp_temp_mean,exp_temp_std, xlabel="Temporal Mean of GFP # (x10^6)",ylabel="Standard Deviation (x10^6)")
...
for i in 1:18
    ...
    plot!(freqs[1:div(end,2)], abs.(fft_result[1:div(end,2)]), xlabel="Frequency (Hz)", ylabel="Magnitude", label="Frequency Spectrum",title="FFT of Cells GFP Expression (n=18)")
end
plot!(yscale=:log10,legend=:false)
fft_res_l = reduce(hcat,fft_res_list)
fft_res_m = mean(fft_res_l,dims=2)
plot(freqs[1:div(end,2)], abs.(fft_res_m[1:div(end,2)]), xlabel="Frequency (Hz)", ylabel="Magnitude", label="Frequency Spectrum",yscale=:log10,title="Mean FFT of Cells GFP Expression (n=18)")
...
Cors = Bkg["Cor #GFP"]; # corrected image traces
OGs = Bkg["OG #GFP"]; # uncorrected image traces
...
#apply linear detrending to uncorrected image traces
detrended_datam = detrend_linear_columns_add_means(OG)
#plot detrended uncorrected time traces
plot();
for i in 1:Cols
