...
dprob = DiscreteProblem(rn, u0, (0.0,4000.0), p);
...
@time for i in 1:10000
    ...
    kTrF_new = bounded_sample(ukTrFg, 1250, 250)
    dG_new = bounded_sample_min0(udG_ln)
    p0 = 1/(kOn_new/kOff_new + kTrOn_new/kTrOff_new + 1.0)
    pA = p0*kOn_new/kOff_new
    pB = p0*kTrOn_new/kTrOff_new
    println(p0," ",pA," ",pB," ",p0+pA+pB)
    kTrTot = pA*kTr_new + pB*kTrF_new
    u0RNA = kTrTot
    u0GFP = kTl_new*u0RNA/dG_new
    r_choice = rand()
    # pick one of the possible starting points for DNA state
    if r_choice <= p0
        u0_new = (DNAoff => 1, DNAon => 0, A => 1, B => 1, DNAonF => 0, RNA => round(Int64,u0RNA), GFP => round(Int64,u0GFP))
    elseif r_choice <= p0+pA
        u0_new = (DNAoff => 0, DNAon => 1, A => 0, B => 1, DNAonF => 0, RNA => round(Int64,u0RNA), GFP => round(Int64,u0GFP))
    else
        u0_new = (DNAoff => 0, DNAon => 0, A => 1, B => 0, DNAonF => 1, RNA => round(Int64,u0RNA), GFP => round(Int64,u0GFP))
    end
    ...
    GFP = sol[7,:][end-215:end]/1000000 #Changed to get values from indexes corresponding to past 1000hrs (steady state) now that there are 3x as many points saved in simulation output (only storing 216 data points) 
    # summary statistics of output w/o noise 
    slope = Polynomials.fit(time, GFP, 1)[1]
    k1 = mean(GFP)
    k2 = var(GFP)
    k3 = mean((GFP.-k1).^3)
    ...
    slopen = Polynomials.fit(time, GFPN, 1)[1]
    cvn = k2n/k1n
