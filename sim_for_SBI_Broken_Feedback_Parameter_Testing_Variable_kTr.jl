...
p = (kOn => 0.00033*60*60, kOff => 0.00033*60*60,  kTr => 180, kTrOn => 0.0001*60*60, kTrOff => 0.0001*60*60, kTrF => 500, kTl => 425, dM => 0.75, dG => 0.00365) #parameter set for NO FEEDBACK Circuit
...
ukTrOn = Normal(0.0001*60*60,0.2*0.0001*60*60)
ukTrOff = Normal(0.0001*60*60, 0.2*0.0001*60*60)
ukTr = Normal(310,150)
...
kTr_g_scale = 60^2/180
kTr_g_shape =180/kTr_g_scale
...
for i in 1:1000
    ...
    kTr_new = bounded_sample(ukTrg,250, 50)
...
jldsave("0_00033_0.75_noise_2kTr2.jld2"; sim_sumstats,simn_sumstats,params)
